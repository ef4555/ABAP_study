# 8/22 yh02(linecolor, emphasize, refresh, event)

생성자: 최예훈
생성 일시: 2023년 8월 22일 오전 10:00
태그: 실습
...
*&---------------------------------------------------------------------*
* 조회화면에서 레이아웃 선택 가능
*TOP-OF-PAGE 추가(현재 날짜, 시간 정보, 플랜트 정보 나타냄)
*데이터 수정/조회 모드 버튼 추가. 수정모드, 조회모드 칼럼, 라인 색 변경(자재 이름 부분만 수정 가능하도록 필드 카탈로그 설정)
*ALV에서 수정한 자재 이름 정보 MAKT에 업데이트
*수정한 값 저장시 ALV REFRESH

REPORT  zppr_bom_yh02.

INCLUDE zppr_bom_yh02_top.
INCLUDE zppr_bom_yh02_cls. " 클래스 정의
INCLUDE zppr_bom_yh02_scr. " 1000번 스크린(조회 조건 화면)
INCLUDE zppr_bom_yh02_pbo. " 스크린 100번의 PBO
INCLUDE zppr_bom_yh02_pai. " 스크린 100번의 PAI
...
DATA : gv_splitter TYPE REF TO cl_gui_splitter_container,
       con1 TYPE REF TO cl_gui_container, " 컨테이너
       con2 TYPE REF TO cl_gui_container, " 컨테이너
       g_dyndoc_id TYPE REF TO cl_dd_document,

       gs_variant TYPE disvariant, " ALV 레이아웃 저장하기 위한 변수
...
DATA : gv_edit_mode TYPE INT4 VALUE 0. " 조회/수정모드 나타내는 변수

DATA : gv_report_name TYPE sdydo_text_element VALUE 'BOM 조회'.

DATA : gv_edit_color(4) TYPE c VALUE 'C500'. " EDIT시 색 변경
```
...
CLASS lcl_event_receiver DEFINITION. " 데이터 변경 이벤트 반응하는 클래스
  PUBLIC SECTION.
    METHODS: handle_data_changed
         FOR EVENT data_changed OF cl_gui_alv_grid " " cl_gui_alv_grid에 정의된 TOP-OF-PAGE 이벤트를 감지 data_changed 이벤트를 감지
...
* 1레벨의 경우에만 기준수량을 가져온다."최상위 인경우 bom 헤더 수량을 가지고 온다
    IF it_list-stufe EQ 1.
      ...
      IF gv_edit_mode = 0. " 조회 모드인 경우
        it_list-linecolor = 'C500'. " row를 초록색으로 변경
      ENDIF.
