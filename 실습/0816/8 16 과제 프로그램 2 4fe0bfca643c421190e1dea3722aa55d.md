...
INCLUDE zppr_bom_yh_frm. " 서브루틴 정리

*----------------------------------------------------------------------*
* INITIALIZATION                                                       *
*----------------------------------------------------------------------*
INITIALIZATION.
  PERFORM init_before_run. " 사용할 인터널 테이블, 구조체 초기화
...
DATA : it_base_str TYPE result_type. " it_base는 헤더라인 없으므로 it_base_str 구조체 생성해주어야 함

TYPES : BEGIN OF sub_bom_matnr,
          matnr TYPE mara-matnr,
          maktx TYPE makt-maktx,
        END OF sub_bom_matnr.

DATA :  it_base_sub TYPE TABLE OF sub_bom_matnr WITH HEADER LINE WITH NON-UNIQUE KEY matnr, " 취득한 it_stpox 기준으로 for all entries in 사용 하여, 제품 기준으로 하위 품목을 저장할 테이블
        it_list TYPE TABLE OF result_type WITH HEADER LINE, " 한 자재번호에 대한 결과값을 담을 인터널 테이블
...
FORM init_before_run.
  CLEAR    :  con1_dock, g_grid1, gs_variant, gs_layout, gs_fieldcat, it_list, it_list[], it_base,  gt_fieldcat.
...
FORM bom_expl.
  ...
  CALL FUNCTION 'CS_BOM_EXPL_MAT_V2'
    EXPORTING
      ...
      emeng                 = 0
      mehrs                 = 'X'
      mtnrv                 = it_base_str-matnr " 상위 자재 번호
      werks                 = p_werks " 플랜트
...
* 4에서 취득한 it_stpox 기준으로 for all entries in 사용 하여, 제품 기준으로 하위 품목을 가지고 온다.
  SELECT ma~matnr kt~maktx
      INTO CORRESPONDING FIELDS OF TABLE it_base_sub
  ...
  LOOP AT it_stpox.
    ...
    it_list-matnr = it_base_str-matnr.
    it_list-maktx = it_base_str-maktx.
    it_list-stlan = it_base_str-stlan.
    it_list-stlal = it_base_str-stlal.
    it_list-stlst = ls_topmat-stlst.
    it_list-stufe = it_stpox-stufe.
    it_list-ojtxp = it_stpox-ojtxp.

    it_list-menge = it_stpox-mngko.    "구성부품수량
    it_list-meins = it_stpox-meins.
    it_list-mtart = it_stpox-mtart.
    it_list-datuv = it_stpox-datuv.
    it_list-stlal = it_stpox-stlal.
    it_list-ausch = it_stpox-ausch.
    it_list-sanka = it_stpox-sanka.
...
*    READ TABLE it_list WITH TABLE KEY ojtxp = it_base-maktx.
    it_list-matnr_m = it_base_sub-matnr.
    MODIFY it_list TRANSPORTING matnr_m WHERE ojtxp = it_base_sub-maktx.
  ENDLOOP.

* 1레벨(최상위)의 경우 기준 수량을 가지고 온다.(BOM 헤더)
  LOOP AT it_list.
    IF it_list-stufe EQ 1.
      it_list-bmeng = ls_topmat-bmeng.
      it_list-bmein = ls_topmat-bmein.
      it_list-linecolor = 'C500'.
      MODIFY TABLE it_list TRANSPORTING bmein bmeng linecolor.
* 1레벨이 아닌 경우 그 위의 모품목 수량을 가지고 온다.
    ELSEIF it_list-stufe NE 1.
      ...
      MODIFY TABLE it_list TRANSPORTING bmein bmeng.
    ENDIF.
  ENDLOOP.

  " 한 자재코드에 해당하는 BOM expl 내역을 출력할 테이블 it_list2에 옮긴다.
  APPEND LINES OF it_list[] TO it_list2[].
