# 8/21 과제 프로그램(수정기능 추가)

생성자: 최예훈
생성 일시: 2023년 8월 21일 오후 5:15
태그: 실습

수정 기능 구현(makt와 연동한 자재 이름 수정) - 자재이름 상위자재가 바뀌었는지 하위자재가 바뀌었는지 구분할 수 없다는게 단점

그래서 불필요하게 row를 두 번 넣는다.

수정에 관한 로직은 class에 있다

수정/조회 모드 바꿀 수 있는 버튼 구현

현재 상태에 따라 top_of_page 변화 구현
...
DATA : gt_modified_rows TYPE TABLE OF makt.

DATA : edit_mode TYPE INT4 VALUE 0.

DATA : lv_name TYPE sdydo_text_element VALUE 'BOM 조회'.
```
...
CLASS lcl_event_receiver IMPLEMENTATION. "구현부
...
*       MESSAGE '데이터 저장 테스트' TYPE 'I'.
    ...
    when 'EDIT'.
      IF edit_mode = 0. " 조회 모드일때 EDIT을 누르면
         edit_mode = 1. " 수정 모드로 변경
      ELSE.
        edit_mode = 0.
      ENDIF.
    WHEN OTHERS.
...
FORM set_top_page  USING  e_dyndoc_id  TYPE REF TO cl_dd_document.
  ...
  lv_today = sy-datum. " 조회 일자
  lv_nowtime = sy-uzeit. " 조회 시간
